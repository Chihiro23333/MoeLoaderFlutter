...
typedef ExceptionActionCallback = void Function(AuthorState authorState);
typedef LoadingStatusBuilder = Widget Function(AuthorState authorState);
  ...
  Widget _buildBody(BuildContext context, AsyncSnapshot snapshot) {
    if (snapshot.connectionState == ConnectionState.active) {
      ...
      if (authorState.error) {
        ...
        if (authorState.code == ValidateResult.needChallenge ||
            authorState.code == ValidateResult.needLogin) {
          ...
          children.add(ElevatedButton(
              onPressed: () {
                ...
                if (actionOnPressed != null) {
                  actionOnPressed(authorState);
                }
              },
              child: Text(tipsByCode(authorState.code))));
        }
      ...
      if (authorState.loading && authorState.list.isEmpty) {
        return const Center(
      ...
      return Padding(
        ...
        child: widget.builder(authorState),
      );
    } else {
      return const Center(
        child: CircularProgressIndicator(),
