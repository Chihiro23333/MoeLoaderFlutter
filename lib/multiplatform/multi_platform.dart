...
class MultiPlatform {
  ...
  Future<Directory> downloadsDirectory() async{return Future.value(Directory(""));}

  Future<Directory> browserCacheDirectory() async{return Future.value(Directory(""));}
  ...
  Widget favicon(Rule rule){return const SizedBox();}
...
class PlatformWindows implements MultiPlatform{
  ...
  Future<Directory> browserCacheDirectory() {
    return Future.value(Directory(path.join(path.current ,Const.dirBrowserCache)));
  }
  ...
  Widget favicon(Rule rule) {
    if (rule.type == Const.typeDefault) {
      return Image.asset("assets/${Const.dirIcons}/${rule.faviconPath}");
    } else {
      Directory imagesDirectory = Global.imagesDirectory;
      return Image.file(File("${imagesDirectory.path}/${rule.faviconPath}"));
    }
  }
